...
server <- function(input, output) {
    output$distPlot <- renderPlot({
        ...
        ggplot(datos,aes(x=a,y=b,size=residual.sugar,color=quality))+geom_point(alpha=0.2)+scale_size(range=c(.1,30))+scale_color_gradient(low = 'blue', high = 'red')#+scale_y_continuous(breaks=seq(0, 3, 0.5))
        #plot(b)
        #plot(data$chlorides)
    ...
    output$distPlottt <- renderPlot({
        ...
        attribute <-input$var
        wine <- subset(wine, quality == input$quality)
        att <- wine[, c(input$var)]
